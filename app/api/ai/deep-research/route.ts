...
export async function POST(request: Request) {
  try {
    ...
    const job = await createResearchJob({ ownerId: owner.uid, chatSessionId, query, settings });
    startResearchWorker(owner.uid, job.id);
    return NextResponse.json({ ok: true, data: { job: toResearchJobView(job) } }, { status: 202 });
  } catch (error) {
