...
const providers: OAuthProviderId[] = ["google", "slack", "github", "notion", "firebase"];
const chatNode = {
  id: "chat:message",
  label: "Conversation",
  type: "chat"
...
function assertStage16Contracts() {
  ...
  for (const provider of providers) {
    getOAuthRedirectUri(provider, "http://localhost:3100/api/oauth/test/connect") satisfies string;
  }
