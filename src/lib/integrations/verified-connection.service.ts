...
export function mergeVerifiedConnectionStates(
  ...
  return AUTOMATION_APPS.map((app) => {
    ...
    if (oauth?.status === "connected" && oauth.verifiedAt) {
      return state(app, "connected", "oauth", oauth.accountLabel || app.label, oauth.verifiedAt, Boolean(oauth.canConnect));
    }
    if (verifiedCredential) {
      return state(app, "connected", "credential", verifiedCredential.accountLabel || verifiedCredential.label, verifiedCredential.verifiedAt || null, true);
    }
    const oauthNeedsReconnect = oauth && ["configured_unverified", "expired", "revoked", "error"].includes(oauth.connectionState || "");
    if (appCredentials.length || oauthNeedsReconnect) {
      return state(app, "needs_reconnect", null, null, null, Boolean(app.credentialFields.length || oauth?.canConnect));
    }
    return state(app, "not_connected", null, null, null, Boolean(app.credentialFields.length || oauth?.canConnect));
  });
}

export async function getVerifiedConnectionStates(ownerId: string, requestUrl?: string) {
  const [{ listCredentials }, { getConnectorAuthState }, { listIntegrationSyncSettings }] = await Promise.all([
    import("../automation/credential.repository"),
    import("./connection-status"),
    import("./integration-settings.repository"),
  ]);
  const [credentials, syncSettings, oauthEntries] = await Promise.all([
  ...
  return mergeVerifiedConnectionStates(credentials, Object.fromEntries(oauthEntries), syncSettings);
}
