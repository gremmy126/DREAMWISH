...
export function AutomationView() {
  ...
  return (
    <div className="space-y-5">
      <Header action={<button type="button" onClick={() => setModalOpen(true)} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-app-primary px-4 text-sm font-semibold text-white shadow-soft"><Plus size={16} />새 자동화</button>} />

      <div className="grid grid-cols-4 gap-4">
        <Metric icon={Play} label="활성 자동화" value={String(rows.filter((row) => row.status === "active").length)} />
        <Metric icon={BarChart3} label="전체 자동화" value={String(rows.length)} />
        <Metric icon={CheckCircle2} label="승인 대기" value="0" />
        <Metric icon={Clock3} label="실행 기록" value={String(rows.reduce((sum, row) => sum + row.runs, 0))} />
      </div>

      <div className="grid grid-cols-[minmax(0,1fr)_360px] gap-5">
        <SurfaceCard className="overflow-hidden">
          <div className="border-b border-app-border px-5 py-4">
            <p className="text-base font-semibold text-app-text">자동화 목록</p>
          </div>
          {rows.length === 0 ? (
            <div className="p-8">
              <EmptyState icon={ScrollText} title="자동화 없음" description="새 자동화를 눌러 Trigger와 Action을 직접 만드세요." />
            </div>
          ) : (
            rows.map((row) => (
        ...
        <SurfaceCard className="p-5">
          <p className="text-lg font-semibold text-app-text">{selected?.name || "선택된 자동화 없음"}</p>
          <p className="mt-2 text-sm leading-6 text-app-muted">
            {selected ? "자동화는 Preview와 Approval 뒤에 실행됩니다." : "자동화를 만들면 상세 설정이 표시됩니다."}
          </p>
          {selected ? (
            <div className="mt-5 space-y-3 text-sm">
              <Detail label="트리거" value={selected.trigger} />
              <Detail label="액션" value={selected.action} />
              <Detail label="상태" value={selected.status} />
              <Detail label="성공률" value={`${selected.successRate}%`} />
            </div>
          ) : null}
        </SurfaceCard>
      </div>
      ...
      {modalOpen ? (
        <Modal title="새 자동화" onClose={() => setModalOpen(false)}>
          <Input label="이름" value={form.name} onChange={(name) => setForm((prev) => ({ ...prev, name }))} />
          <Input label="트리거" value={form.trigger} onChange={(trigger) => setForm((prev) => ({ ...prev, trigger }))} />
          <Input label="액션" value={form.action} onChange={(action) => setForm((prev) => ({ ...prev, action }))} />
          <button type="button" onClick={() => void createAutomation()} className="mt-3 h-11 w-full rounded-app bg-app-primary text-sm font-semibold text-white">저장</button>
          <AutomationHelp
...
function Header({ action }: { action: React.ReactNode }) {
  return <div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-semibold text-app-text">Automation</h1><p className="mt-2 text-sm text-app-muted">직접 만든 자동화와 승인 기반 실행 흐름을 관리합니다.</p></div>{action}</div>;
}
...
function AutomationHelp({
  ...
  return (
    <div className="mt-3 space-y-3 rounded-app border border-app-border bg-app-bg p-3">
      <HelpGroup label="Trigger" description={triggerHelp} examples={triggerExamples} onPick={onTrigger} />
      <HelpGroup label="Action" description={actionHelp} examples={actionExamples} onPick={onAction} />
      <div>
        <p className="text-xs font-semibold text-app-text">Preview flow</p>
