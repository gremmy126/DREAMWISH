  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-200 p-3">
      ...
      {definition.inputSchema.fields.filter((field) => isVisible(field, value)).map((field) => (
        <ActionField key={field.id} field={field} value={value[field.id]} onChange={(next) => patch(field.id, next)} />
      ))}
...
function ActionField({
  ...
  if (field.type === "boolean") {
    return <label className="flex items-center gap-2">{label}<input type="checkbox" checked={value === true} onChange={(event) => onChange(event.target.checked)} className="accent-violet-600" /></label>;
  }
  if (field.type === "select") {
    return <label className="block">{label}<select className={base} value={typeof value === "string" ? value : ""} onChange={(event) => onChange(event.target.value)}><option value="">선택</option>{field.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{help(field)}</label>;
  }
  if (field.type === "multiselect") {
    const selected = Array.isArray(value) ? value.map(String) : [];
    return <label className="block">{label}<select multiple className={`${base} min-h-24 py-2`} value={selected} onChange={(event) => onChange(Array.from(event.target.selectedOptions, (option) => option.value))}>{field.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{help(field)}</label>;
  }
  if (["json", "key_value", "array", "mapping"].includes(field.type)) {
    return <label className="block">{label}<textarea className={`${base} min-h-24 py-2 font-mono`} placeholder={field.placeholder || "JSON"} value={toEditableJson(value)} onChange={(event) => onChange(parseJsonOrText(event.target.value))} />{help(field)}</label>;
  }
  if (field.type === "textarea") {
    return <label className="block">{label}<textarea className={`${base} min-h-24 py-2`} placeholder={field.placeholder} value={typeof value === "string" ? value : ""} onChange={(event) => onChange(event.target.value)} />{help(field)}</label>;
  }
