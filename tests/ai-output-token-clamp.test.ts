...
// llama-3.1-8b 8192, gemma-2-2b 등)은 한도를 넘는 값을 400으로 거부하므로,
// 공급자별 상한으로 잘라내야 모든 등록 모델이 Agent 생성에 사용될 수 있다.
test("output token requests are clamped to each model's limit", () => {
  ...
  assert.equal(clampOutputTokens(16_000, 4_000), 4_000); // huggingface
