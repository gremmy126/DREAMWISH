...
**Goal:** 기존 DREAMWISH 사이드바와 상단바를 그대로 유지하면서 비즈니스에 실제 ERPNext 데이터만 표시하는 반응형 `ERP 대시보드` 탭을 추가하고, 기존 `영업·매출`·비즈니스 휴대폰 매출 UI와 캘린더의 `휴대폰에서 가져오기` UI를 제거한다.
...
- 공유 필드명은 `connectionState`와 `connectionMode`다. mode는 `server | local_gateway | null`이며 `local`, `status`, `source` alias를 만들지 않는다. freshness는 별도 `stale` flag다.
...
- 선행 의존성: `docs/superpowers/plans/2026-07-15-automation-connection-binding.md`를 먼저 완료한다. ERPNext credential route를 확장할 때 typed scenario binding, unified safe candidates, exact owner/app resolver를 보존하고 integration-only ERPNext app을 scenario 후보로 노출하지 않는다.
...
test("ERP dashboard keeps verified zero distinct from an unavailable value", () => {
  const snapshot = normalizeErpDashboardSnapshot({
    ...
    accountingPeriod: { fromDate: "2026-07-01", toDate: "2026-07-31", timeZone: "Asia/Seoul" },
    currency: "KRW",
...
- connection defaults는 `connectionMode: "server"`, `connectionRevision: 1`, `capabilityVersion: 1`, `customer_search: true`, `customer_read: true`, `draft_write: false`이고 disconnected mode는 `null`이다.
...
export type ErpConnectionState = "not_configured" | "connected" | "degraded" | "error";
export type ErpConnectionMode = "server" | "local_gateway" | null;
...
- verified save는 credential과 별도로 `connectionRevision: 1`, `capabilityVersion: 1` metadata를 만들고 read capabilities true, `draft_write` false로 저장한다. reconnect/site/company/credential 변경은 `connectionRevision`을 증가시키고 안전하게 `draft_write: false`로 되돌리면서 `capabilityVersion`도 증가시킨다. capability-only 변경은 `connectionRevision`을 건드리지 않는다. disconnect는 metadata를 revoke한다.
